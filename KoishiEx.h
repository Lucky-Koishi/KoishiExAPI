/*
KoishiEx.h - DNF多媒体压缩包解析库

该解析库可以用于对DNF的ImagePack2目录下的NPK文件和衍生出的IMG文件进行解析、
浏览、提取和修改等操作，利用解析库可以制作属于自己的EX工具，也可以制作出具
有特定功能的与多媒体文件有关的工具（例如一键替换贴图、纸娃娃系统等）。
目前解析库可解析版本号为2、4、5、6的IMG文件，并支持这些版本中间的某些特定
的转换工作。
该解析库依赖于libpng15和zlib两个静态库，闭源状态下使用该解析库时，有可能需
要对项目的默认库输入进行设置，具体情况可参考同目录下的readme.txt。

作者声明：
本解析库按原样提供，不会有任何明示或者暗示性质的保证，原作者将不会承担任何
来自于该解析库所造成的风险和损失；允许本解析库的使用者将其用于商业用途，或
者对其进行二次开发，并无需告知原作者，但我们希望广大使用者积极寻找BUG和对
解析库提出宝贵的改进意见，请将您宝贵的意见发送到邮箱swablu@qq.com。

原作者：乘着歌声的翅膀（恋恋）于2017年11月15日
源代码网址：https://github.com/Lucky-Koishi/KoishiEx
Github主页：https://github.com/Lucky-Koishi
*/
